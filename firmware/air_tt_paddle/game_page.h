...
const state = {
  ...
  paused: true,
  dancing: false,
  playerHandled: false, cpuHandled: false, cpuWillMiss: false,
}

// First to this many points wins.
const WIN_SCORE = 5;

// How often the opponent lets a ball go — the main difficulty dial, kept in
// one place because it was previously duplicated at the serve and at the
// player's return and could drift apart. Raise BASE to make the CPU easier,
// lower it to make it tougher.
const CPU_MISS_BASE = 0.15;   // chance at the start of a rally
const CPU_MISS_RAMP = 0.04;   // extra chance per unit of rally speed gained
const CPU_MISS_MAX  = 0.30;   // never a total pushover, even in a long rally
function cpuMissChance() {
  return Math.min(CPU_MISS_MAX,
                  CPU_MISS_BASE + Math.max(0, state.rallySpeed - 1.6) * CPU_MISS_RAMP);
}
...
function serve(who) {
  ...
  // A serve from you is a ball the opponent has to receive, so it needs the
  // same up-front decision a returned ball gets.
  state.cpuWillMiss = (who === 'you') ? Math.random() < cpuMissChance() : false;
  b.active = true;
  state.paused = false;
}
...
function pointTo(who) {
  ...
  if (state.scoreYou >= WIN_SCORE || state.scoreCpu >= WIN_SCORE) { endGame(); return; }
...
function step(dt) {
  ...
  // The net is drawn but is deliberately NOT a collider. With a single-axis
  // paddle you have no control over shot height, so a net that blocks low
  // balls punishes you for something you cannot aim — it read as unavoidable
  // rather than as a skill check.
  ...
  if (b.z <= PADDLE_Z + 0.06 && b.vz < 0 && b.z > PADDLE_Z - 0.45 && !state.playerHandled) {
    ...
    if (Math.abs(dx) < PADDLE_HIT_RADIUS_X) {
      ...
      // Decide NOW, at the moment you return the ball, not on contact. The
      // opponent then stops tracking for this shot, so a miss looks like it
      // let the ball go rather than the ball passing through a bat that was
      // sitting right on it.
      state.cpuWillMiss = Math.random() < cpuMissChance();
    }
  }
  ...
  // When it has already decided to let this one go, it stops chasing and
  // eases back to centre, so the miss is visible as a choice.
  if (state.cpuWillMiss && b.vz > 0) {
    state.cpuX += (0 - state.cpuX) * 1.2 * dt;
  } else {
