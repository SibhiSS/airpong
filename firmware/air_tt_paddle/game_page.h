...
function serve(who) {
  ...
  state.cpuWillMiss = (who === 'you')
    ? Math.random() < Math.min(0.55, 0.28 + Math.max(0, state.rallySpeed - 1.6) * 0.06)
    : false;
  b.active = true;
  state.paused = false;
}
...
function step(dt) {
  ...
  if (b.z <= PADDLE_Z + 0.06 && b.vz < 0 && b.z > PADDLE_Z - 0.45 && !state.playerHandled) {
    ...
    if (Math.abs(dx) < PADDLE_HIT_RADIUS_X) {
      ...
      state.cpuWillMiss = Math.random() < missChance;
    }
  }
